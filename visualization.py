...
def plot_embedding(X, query_sample, target_num, title=None):
    ...
    #     if target_num[i] <= 50:
    #         s1 = plt.scatter(X[i, 0], X[i, 1],s=0.5,c='lightcyan')#(3840, 2)

    #     elif target_num[i] <= 100:
    #         s2 = plt.scatter(X[i, 0], X[i, 1],s=0.5,c='paleturquoise')#(3840, 2)

    #     elif target_num[i] <= 500:
    #         s3 = plt.scatter(X[i, 0], X[i, 1],s=0.5,c='c')#(3840, 2)

    #     elif target_num[i] <= 1000:
    #         s4 = plt.scatter(X[i, 0], X[i, 1],s=0.5,c='cornflowerblue')#(3840, 2)

    #     else: 
    #         s5 = plt.scatter(X[i, 0], X[i, 1],s=0.5,c='royalblue')#(3840, 2)

    # X=X.sample(20000)
    # plt.scatter(X[:, 0], X[:, 1],s=0.5,c='#87CEFA',label='dataset')#(3840, 2)

    for i in range(len(query_sample)):
        query = X[query_sample[i]]#(100, 2)

        adversarial = X[-500:]
        s5 = plt.scatter(query[:,0], query[:,1],s=0.5,c='r',marker='*')
        s6 = plt.scatter(adversarial[:,0], adversarial[:,1],s=0.5,c='orange',marker='D',label='adversarial sample')
        plt.xticks([]), plt.yticks([])
        plt.title(title)

    # time_str = time.strftime("%Y%m%d") + time.strftime("_%H%M%S")
    plt.savefig("picture/" + str(i) + ".png", dpi=1500)
    plt.legend((s5,s6),('selected sample','generated sample'))
...
def visualiazation(X_train, query_sample, target_num, n, strategy_name):
    
    X_train = torch.tensor((X_train.reshape(X_train.shape[0], 128*128).astype('float32')) / 255.0)
    X_tsne = tSNE(X_train,query_sample,'x')

    plot_embedding(X_tsne, query_sample, target_num, "tSNE-"+strategy_name)
    print("finished!!!")
